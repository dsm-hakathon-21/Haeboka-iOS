...
private extension TranslateViewController {
    ...
    func api() {
        let provider = MoyaProvider<EducationAPI>(plugins: [MoyaLoggerPlugin()])
        
        provider.request(.translation(input: sourceLabel.text!, source: Language.ko.rawValue, target: Language.en.rawValue)) { res in
            switch res {
            case .success(let result):
                switch result.statusCode {
                case 200...299:
                    if let data = try? JSONDecoder().decode(TranslateReponse.self, from: result.data) {
                        self.resultLabel.text = data.translatedText
                    }
                default:
                    print("Fail")
                }
