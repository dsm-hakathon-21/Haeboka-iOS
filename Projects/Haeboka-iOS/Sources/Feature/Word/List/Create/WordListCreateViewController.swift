...
class WordListCreateViewController: BaseVC {
    
    let storage: WordStorageType? = nil
    ...
    public override func bind() {
        ...
        createButton.rx.tap
            .bind(onNext: { [weak self] in
                self?.storage?.createWord(definition: self?.wordTextField.text ?? "", meaning: self?.meaningTextField.text ?? "")
                self?.dismiss(animated: true)
            })
            .disposed(by: disposeBag)
